...
public class ScriptGenerator {
	...
	public static String generateChExternalDir(RDBSchema schema,
		...
		if (externalLoc.isEmpty() == false) {
			...
			if (externalGroup.isEmpty() == false) {
				builder.append("hadoop fs -chgrp " + externalGroup + " "
						+ externalLoc + "/" + schema.getTableName() + ls + ls);
			}
			if (externalPermission.isEmpty() == false) {
				builder.append("hadoop fs -chmod " + externalPermission + " "
						+ externalLoc + "/" + schema.getTableName() + ls + ls);
			}
	...
	public static String generateHiveTable(RDBSchema schema,
			String externalLocation, String database) {
		...
		builder.append("TABLE " + database + "." + schema.getTableName()
				+ lineSeparator);
		builder.append("( ");
		...
		if (externalLocation != null && externalLocation.isEmpty() == false) {
			builder.append(" LOCATION \\\"" + externalLocation + "/"
					+ schema.getTableName() + "\\\"" + lineSeparator);
		}
	...
	public static String generateTempHiveTable(RDBSchema schema,
			String externalLocation, String addJors, String rowFormat,
			String tempTableStoredAs) {
		return generateBasicStringHiveTable(schema, externalLocation, addJors,
				Const.TEMP_POSTFIX, rowFormat, tempTableStoredAs);
	}

	public static String generateBackPortHiveTable(RDBSchema schema,
			String externalLocation, String addJars, String rowFormat) {
		return generateBasicStringHiveTable(schema, externalLocation, addJars,
				Const.BACKPORT_POSTFIX, rowFormat, "STORED AS TEXTFILE");
	}

	public static String generateBasicStringHiveTable(RDBSchema schema,
			String externalLocation, String addJars, String postFix,
			String rowFormat, String storedAs) {
		...
		if (addJars.isEmpty() == false) {
			builder.append("ADD JAR " + addJars + ";" + lineSeparator
					+ lineSeparator);
		}
		...
		builder.append("TABLE " + schema.getTableName() + postFix
				+ lineSeparator);
		builder.append("( ");
		...
		if (externalLocation.isEmpty() == false) {
			builder.append(" LOCATION \\\"" + externalLocation + "/"
					+ schema.getTableName() + postFix + "\\\"" + lineSeparator);
		}
	...
	public static String generateDropTempHiveTable(RDBSchema schema,
		...
		builder.append("hive -e \"DROP TABLE " + schema.getTableName()
				+ Const.TEMP_POSTFIX + ";\"");
	...
	public static String generateDeleteTempTableCommand(RDBSchema schema,
		...
		String deleteTempFolder = prop.getProperty(
				Const.DELETE_TEMP_TABLE_DATA_AFTER_LOAD, "false");
		String rootExternalLocation = prop.getProperty(
				Const.ROOT_EXTERNAL_LOCATION, "");
		String insertIntoMode = prop.getProperty(Const.INSERT_INTO_LOGIC,
				Const.INSERT_INTO_LOGIC_NORMAL);
		String newLine = System.getProperty("line.separator");

		StringBuilder builder = new StringBuilder();

		if (deleteTempFolder.equals("true")
				&& rootExternalLocation.isEmpty() == false) {

			if (insertIntoMode.equals(Const.INSERT_INTO_LOGIC_HIVE8_SIM)) {
				builder.append(newLine + "hadoop fs -rm -r -skipTrash "
						+ rootExternalLocation + "/" + schema.getTableName()
						+ Const.EXISTING_TEMP_POST_DIR_NAME + newLine);
				...
				builder.append(newLine + "hadoop fs -rm -r -skipTrash "
						+ rootExternalLocation + "/" + schema.getTableName()
						+ Const.DELTA_TEMP_POST_DIR_NAME + newLine);
			}
			builder.append(newLine + "hadoop fs -rm -r -skipTrash "
					+ rootExternalLocation + "/" + schema.getTableName()
					+ Const.TEMP_POSTFIX + newLine);
		}
		return builder.toString();
	}
	...
	public static String generateInsertInto(RDBSchema schema, Properties prop) {
		...
		String otherDateFormat = prop.getProperty(Const.OTHER_DATE_FORMAT,
				"dd/MM/yy");
		String datetimeFormat = prop.getProperty(Const.DATETIME_FORMAT,
				"dd/MM/yyyy HH:mm:ss");
		boolean applyTeradataFormatting = prop.getProperty(
				Const.APPLY_TERADATA_FORMATTING, "false").equals("true");

		boolean doTrimOnString = prop.getProperty(Const.TRIM_STRING_VALUES,
				"false").equals("true");
		String compressionCodec = prop.getProperty(Const.COMPRESSION_CODEC,
				"org.apache.hadoop.io.compress.SnappyCodec");

		String blockSize = prop.getProperty(Const.FS_LOCAL_BLOCK_SIZE,
				"67108864");

		StringBuilder builder = new StringBuilder();

		builder.append("SET hive.exec.dynamic.partition=true;" + lineSeparator);
		builder.append("SET hive.exec.dynamic.partition.mode=nonstrict;"
				+ lineSeparator);
		...
		builder.append("SET mapred.output.compression.codec = "
				+ compressionCodec + ";" + lineSeparator);
		builder.append("SET fs.local.block.size = " + blockSize + ";"
				+ lineSeparator);
		builder.append("SET hive.io.rcfile.record.buffer.size = "
				+ Math.max((4 * 1024 * 1024),
						((schema.getColumns().size() / 3) * 1024 * 1024)) + ";"
				+ lineSeparator);
		builder.append(lineSeparator);
		...
		if (columns.size() > 0) {
			...
			for (Column c : partitionColumns) {
				...
				appendColumnForInsertIntoFromTemp(dateFormat, otherDateFormat,
						datetimeFormat, applyTeradataFormatting,
						doTrimOnString, builder, c);
			}

			builder.append(lineSeparator);
		}

		builder.append("FROM " + schema.getTableName() + Const.TEMP_POSTFIX
				+ " a ");

		if (prop.getProperty(Const.CHECK_FOR_HEADER_RECORDS, "false").equals(
				"true")) {
			builder.append(lineSeparator);
			builder.append("WHERE a." + columns.get(0).getName() + " != '"
	...
	private static void appendColumnForInsertIntoFromTemp(String dateFormat,
		...
		if (type.equals("VARCHAR2") || type.equals("CHAR")
			...
			builder.append("from_unixtime(unix_timestamp(" + fieldName + ", '"
					+ dateFormat + "'))");
			...
			if (c.getPercision() == 0) {
				...
				if (applyTeradataFormatting) {
					...
					if (type.equals("DECIMAL")) {
						fieldName = "regexp_replace(" + fieldName
								+ ",\\\"\\\\\\.[0-9]*\\\",\\\"\\\")";
					}
				}
				if (c.getLength() > 18) {
	...
	public static String generateLoad(RDBSchema schema, Properties prop) {
		...
		if (isLoadExternal == null || isLoadExternal.isEmpty()
				|| isLoadExternal.toLowerCase().equals("true") == false) {
			return "FILES=$@ "
					+ newLine
					+ "for f in $FILES "
					+ newLine
	...
	public static String generateTestData(RDBSchema schema, Properties prop,
			int startingLine, int linesOfData, char strChar) {
		...
		SimpleDateFormat simpleOtherDateFormat = new SimpleDateFormat(
				otherDateFormat);
		...
		for (int i = startingLine; i < linesOfData; i++) {
			...
			for (Column c : columns) {
				...
				if (c.isPartition()) {
					// If the field is a partition field the following code with
					// generate a partition for every 5 rows.
					String type = c.getType().toUpperCase();
					if (type.equals("DATE")) {
					...
					String type = c.getType().toUpperCase();
					if (type.equals("VARCHAR2") || type.equals("CHAR")
							|| type.equals("CLOB") || type.equals("VARCHAR")
							|| type.equals("CLOB")) {

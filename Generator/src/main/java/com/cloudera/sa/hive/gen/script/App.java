...
public class App 
{
            ...
            if (isHeaderRow) {
            	...
            	if(currentTable.equals(tableName) == false) { 
            		if (currentTable.isEmpty() == false) {
            			tableFileOutput(schema, outputDirector);
            			tableConsoleOutput(schema);
            		}
    ...
    private static void tableFileOutput(RDBSchema schema, String directory) throws IOException {
    	...
    	FileWriter writerD = new FileWriter(new File(directory + "/" + schema.getTableName() + "_SampleData.sh"));
    	
    	ScriptGenerator.lineSeparator = " ";
    	
    	String ls = System.getProperty("line.separator");
    	
    	writerC.write("hive -e \"" + ScriptGenerator.generateHiveTable(schema) + "\"");
    	
    	writerL.write("hive -e \"" + ScriptGenerator.generateTempHiveTable(schema) + "\"");
    	writerL.write(ls + ls);
    	writerL.write("hive -e \"" + ScriptGenerator.generateLoadOverwrite(schema) + "\"");
    	writerL.write(ls + ls);
	...
	private static void tableConsoleOutput(RDBSchema schema) {

    	ScriptGenerator.lineSeparator = System.getProperty("line.separator");
		System.out.println("----------------------");
		System.out.println("-- Step1: Create Hive Temp Table");
        System.out.println(ScriptGenerator.generateTempHiveTable(schema));
        System.out.println();
        System.out.println("-- Step2: Create Hive Table");
        System.out.println(ScriptGenerator.generateHiveTable(schema));
        System.out.println();
